...
export default function DashboardHeader({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parts.length >= 2) {
        ...
        const payload = JSON.parse(jsonPayload) as { id?: string; userId?: string; email?: string; userEmail?: string; sub?: string };
        const emailFromJwt = payload.email ?? payload.userEmail ?? payload.sub ?? null;
        const idFromJwt = payload.id ?? payload.userId ?? payload.sub ?? null;
        if (emailFromJwt) setUserEmail(emailFromJwt);
        if (idFromJwt) setUserId(idFromJwt);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    // Verificar resumo do dia (primeiro login)
    crmService.loginCheck().catch(console.error);
