...
export default function Page() {
  ...
  const handleEnviarCodBarras = useCallback(async () => {
    ...
    try {
      ...
      toast('Código de barras adicionado com sucesso!', 'success');
      setAddBarrasOpen(false);

      lastBarrasReqRef.current = null;
      await fetchCodBarras(String(codProdNum));
    } catch (e: any) {
      setAddBarrasErr(e.message || 'Falha ao adicionar código de barras.');
    } finally {
      setAddBarrasLoading(false);
    }
  }, [CRIAR_COD_BARRAS_URL, codBarras, getHeaders, produto?.CODPROD, fetchCodBarras, toast]);


  // Modal Notificar Erro Estoque
  ...
  const handleEnviarErroEstoque = useCallback(async () => {
    ...
    try {
      ...
      setErroEstoqueErr(e.message || 'Erro ao notificar erro.');
    } finally {
  ...
  return (
    <DashboardLayout subtitle="Consulta de Produto &amp; Estoque">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-100 bg-emerald-50/30">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-bold text-emerald-900">Consulta de Produtos</h2>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              ...
              <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none px-4 py-2 bg-white border border-slate-300 hover:bg-rose-50 hover:border-rose-300 hover:text-rose-600 text-slate-600 rounded-lg font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
                >
            ...
            <div className="p-6 bg-slate-50/50 animate-fade-in-up">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  {/* Campos Read-only para Consulta */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Localização</label>
                      <div className="px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm font-mono text-slate-700">
                        {localizacao || '-'}
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Qtd. Máxima</label>
                      <div className="px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm font-mono text-slate-700">
                        {AD_QTDMAX || '-'}
                      </div>
                    </div>
                  ...
                  <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {hasMoreBarras && (
                        ...
                        >
                          {barrasExpanded ? <>Minimizar <ChevronUp className="w-3.5 h-3.5" /></> : <>Expandir <ChevronDown className="w-3.5 h-3.5" /></>}
                        </button>
                      )}
                      <button
                        onClick={openAddBarras}
                        className="px-3 py-1.5 bg-white border border-emerald-600 text-emerald-700 hover:bg-emerald-50 rounded-md font-bold text-xs transition-all flex items-center gap-1.5 shadow-sm focus:outline-none"
                      >
                        <Plus className="w-3 h-3" /> Adicionar Cód.
                      </button>
                    </div>
                  </div>
                  <div className="p-4">
                      ...
                      <table className="min-w-full divide-y divide-slate-100">
                        <thead className="bg-slate-50">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cód. Emp</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {produto.estoque.map((it, idx) => (
                            <tr key={`${it.CODLOCAL}-${idx}`} className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="px-4 py-2.5 text-sm text-slate-600">{it.CODEMP ?? '-'}</td>
                        ...
                        <tfoot className="bg-emerald-50/50 border-t-2 border-emerald-100">
                          <tr>
                            <td colSpan={3} className="px-4 py-3 text-right text-xs font-bold text-emerald-900 uppercase tracking-wider">Totais</td>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in-up">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  value={codBarras}
                  onChange={(e) => setCodBarras(onlyNumber(e.target.value))}
                  autoFocus
            ...
            <div className="px-5 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
              ...
              <button
                onClick={handleEnviarCodBarras}
                disabled={addBarrasLoading || !codBarras.trim()}
