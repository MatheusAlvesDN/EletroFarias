...
const calcularResumoEntradas = (entradasData: NotaEntrada[]) => {
  ...
  entradasValidas.forEach((r) => {
    ...
    if (ufStr === 'PB') {
      target = pb;
    } else if (estadosSulSudeste.includes(ufStr)) {
      target = sul;
    } else {
      target = nne;
    }
...
export default function RelatorioUnificado() {
  ...
  const fetchNotas = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const [resAtual, resAnterior, resEntradas, resEntradasAnterior] = await Promise.all([
        ...
        fetch(`${API_BASE}/sankhya/notas-entradas?${qsEntradas}`, {
          headers: { Accept: 'application/json' },
          cache: 'no-store'
        }).catch(() => null),
        fetch(`${API_BASE}/sankhya/notas-entradas?${qsEntradasAnterior}`, {
          headers: { Accept: 'application/json' },
          cache: 'no-store'
        }).catch(() => null)
      ]);
  ...
  const resumoEntradasAnteriorAgrupado = useMemo(() => calcularResumoEntradas(entradasAnteriorDataGlob), [entradasAnteriorDataGlob]);
  ...
  // ATUALIZADO: Ratear ICMS do Livro no fechamento TARE para refletir apenas a porção tributada
  const entradasCfopResumo = useMemo(() => {
    ...
    entradasDataGlob
      ...
      .forEach((r) => {
        ...
        // Rateia o crédito na mesma proporção matemática
        let icmsRateado = icmsLivro;
        if (totalBase > 0) {
          icmsRateado = icmsLivro * (baseTrib / totalBase);
        }
