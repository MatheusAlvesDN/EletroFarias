 ...
 */
type ResponsiveWrapperProps = Omit<ResponsiveProps, 'width'> & { className?: string };
...
export default function DashboardSankhya() {
  ...
  const renderContent = (w: WidgetConfig) => {
    ...
    // (restante do seu renderContent segue igual)
    // ... MANTIVE SUA LÓGICA INTACTA ...

    // ⚠️ Para não explodir a resposta com o arquivo duplicado inteiro,
    // você pode colar aqui o restante do arquivo exatamente como está,
    // a partir do switch(w.type) até o final, sem mudar nada.
    return null;
  };
...
const onLayoutChange = useCallback((currentLayout: Layout, _allLayouts: AllLayouts) => {
  setWidgets((prev) => applyLayoutToWidgets(prev, currentLayout));
}, []);
  // (restante do JSX do return segue igual ao seu)
return (
      ...
      <header className="bg-emerald-700 text-white shadow-lg sticky top-0 z-30">
        <div className="w-full max-w-[1920px] mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
            ...
            {/* ✅ BOTÃO ATUALIZAR (ao lado de Adicionar) */}
            <button
              onClick={() => {
        ...
        {widgets.length > 0 && (
          <div className="bg-transparent">
            <ResponsiveGridLayoutWrapper
              ...
              rowHeight={28}
              compactType={null} // deixa mais livre
              preventCollision={false} // se quiser impedir sobreposição, troque para true
              isResizable
              isDraggable
            ...
            >
              {widgets.map((w) => (
                <div key={w.id} className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                  {/* Header do widget: arraste SOMENTE pela área com .widget-drag-handle */}
                  <div className="px-4 py-3 border-b border-slate-200 bg-emerald-50 flex justify-between items-center gap-2 shrink-0">
                    <div className="widget-drag-handle flex items-center gap-2 font-bold text-emerald-900 cursor-move select-none">
                      <GripHorizontal className="w-4 h-4 text-emerald-600/50" />
