...
// CORREÇÃO: Adicionado 'entrada' ao tipo Visao
...
// CORREÇÃO: Tipagem ajustada para aceitar atributos HTML padrão
const TableHeader = ({ 
  children, 
  align = 'left', 
  onFilter, 
...
const FloatingWidget = ({ widget, updateWidget, bringToFront, children, headerExtra }: any) => {
    const [isDragging, setIsDragging] = useState(false);
    const [pos, setPos] = useState({ x: widget.floatX, y: widget.floatY });
    const [rel, setRel] = useState({ x: 0, y: 0 });

    const handleMouseDown = (e: any) => {
        // Ignora drag se clicar em botões, inputs, etc.
    ...
    const handleMouseUpResize = (e: any) => {
        // Atualiza a config quando o usuário soltar o mouse após o resize do CSS
    ...
    return (
        ...
        >
            {/* Header Dragável */}
            <div 
                className="px-5 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center cursor-move"
...
export default function DashboardSankhya() {
  ...
  // Estados Modal XML Original (Manteve-se)
  ...
  // --- Sistema Dinâmico de Layout (Widgets) ---
  const [maxZ, setMaxZ] = useState(100);
  const [widgets, setWidgets] = useState<WidgetConfig[]>([
      { id: 'entrada', title: 'Faturamento Entrada (Mês Passado)', icon: TrendingUp, order: 0, colSpan: 1, isFloating: false, floatX: 50, floatY: 100, floatW: 600, floatH: 450, zIndex: 100 },
      { id: 'saida', title: 'Faturamento Saída por TOP', icon: TrendingDown, order: 1, colSpan: 1, isFloating: false, floatX: 100, floatY: 150, floatW: 600, floatH: 450, zIndex: 100 },
      { id: 'tipo', title: 'Totais por Tipo', icon: Filter, order: 2, colSpan: 2, isFloating: false, floatX: 150, floatY: 200, floatW: 800, floatH: 450, zIndex: 100 },
      { id: 'parceiros', title: 'Resumo por Parceiro', icon: LayoutDashboard, order: 3, colSpan: 2, isFloating: false, floatX: 200, floatY: 100, floatW: 900, floatH: 600, zIndex: 100 },
      { id: 'xml', title: 'Resultados XML NFe', icon: FileCode2, order: 4, colSpan: 2, isFloating: false, floatX: 250, floatY: 150, floatW: 800, floatH: 600, zIndex: 100 }
  ]);
  ...
  const moveWidget = (id: string, dir: number) => {
      setWidgets(prev => {
          ...
          if (idx < 0 || idx + dir < 0 || idx + dir >= arr.length) return prev;
          
          // Swap orders
          const tempOrder = arr[idx].order;
          arr[idx].order = arr[idx + dir].order;
          arr[idx + dir].order = tempOrder;
          return arr;
      });
  };
  ...
  const changeWidgetWidth = (id: string, delta: number) => {
      setWidgets(prev => prev.map(w => {
          if (w.id === id) {
              const newSpan = Math.max(1, Math.min(3, w.colSpan + delta)); // Max 3 columns
  ...
  const fetchVisao = useCallback(async (visao: Visao, codParc?: number) => {
  ...
  const loadAll = useCallback(() => {
    const run = async () => {
      setLoading(true); setError(null); setSelectedParc(null); setDataDetalhe([]); setXmlPage(0);
      try {
        ...
        const [entradaRaw, topRaw, tipoRaw, parcRaw, xmlRes] = await Promise.all([
          fetchVisao('entrada'), fetchVisao('top'), fetchVisao('tipo'), fetchVisao('parceiro'),
          fetch(`${XML_URL}?${xmlQs}`, { headers: { Accept: 'application/json' }, cache: 'no-store' }).catch(() => null)
        ]);

        setDataEntradaTop(entradaRaw.map((r) => ({ TOPS: String(r.TOPS ?? ''), QTD_NOTAS: toNumber(r.QTD_NOTAS), DESCRICAO: String(r.DESCRICAO ?? ''), VLR_TOTAL_ST: toNumber(r.VLR_TOTAL_ST), VLR_TOTAL_TB: toNumber(r.VLR_TOTAL_TB), VLR_TOTAL: toNumber(r.VLR_TOTAL) })));
        ...
        if (xmlRes && xmlRes.ok) {
           ...
           setXmlRows(Array.isArray(xmlData) ? xmlData : Array.isArray(xmlData?.data) ? xmlData.data : []);
        } else setXmlRows([]);
      } catch (e: any) {
        console.error(e); setError(e?.message ?? 'Ocorreu um erro.');
  ...
  const xmlFiltered = useMemo(() => {
    ...
    return xmlRows.filter((r) => `${safeString(r.NUMNOTA)} ${safeString(r.VLRNOTA)}`.toLowerCase().includes(query));
  }, [xmlRows, xmlQ]);
  ...
  // Filtros Parceiros
  const perfisFat = useMemo(() => Array.from(new Set(dataParc.map(p => p.AD_TIPOCLIENTEFATURAR).filter(Boolean))).sort(), [dataParc]);
  ...
  const applyColumnFilter = () => {
      if (activeFilterCol === 'PERFIL') setPerfilFilter(modalPerfil);
          ...
          if ((modalMin === '' || modalMin === '0') && (modalMax === '' || Number(modalMax) >= currentMaxForSlider)) {
  ...
  // --- RENDERIZADORES DOS CONTEÚDOS DOS WIDGETS ---
            ...
            return (
                <div className="overflow-auto h-full scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 flex-1">
                    <table className="min-w-full divide-y divide-slate-100">
                        ...
                        {dataEntradaTop.length === 0 && !loading && <tr><td colSpan={6} className="p-12 text-center text-slate-400 text-sm italic">Sem dados de entrada para o período.</td></tr>}
                        </tbody>
                    </table>
                </div>
            );
            ...
            return (
                <div className="overflow-auto h-full scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 flex-1">
                    <table className="min-w-full divide-y divide-slate-100">
                        ...
                        {dataTop.length === 0 && !loading && <tr><td colSpan={6} className="p-12 text-center text-slate-400 text-sm italic">Sem dados de saída para o período.</td></tr>}
                        </tbody>
                    </table>
                </div>
            );
            ...
            return (
                <div className="overflow-auto h-full scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 flex-1">
                    <table className="min-w-full divide-y divide-slate-100">
                        ...
                        {dataTipo.length === 0 && !loading && <tr><td colSpan={7} className="p-12 text-center text-slate-400 text-sm italic">Sem dados de tipo.</td></tr>}
                        </tbody>
                    </table>
                </div>
            );
                ...
                <div className="flex flex-col h-full flex-1 min-h-0">
                    <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 flex items-center justify-between gap-4 shrink-0">
                        <div className="relative flex-1">
                            ...
                            <input type="text" value={xmlQ} onChange={(e) => { setXmlQ(e.target.value); setXmlPage(0); }} placeholder="Buscar (NUMNOTA/VLR)..." className="w-full pl-9 pr-4 py-1.5 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none text-sm shadow-inner" />
                        </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-100">
                            ...
                            <tbody className="divide-y divide-slate-50 bg-white">
                                {loading && xmlPageRows.length === 0 ? (
                                    <tr><td colSpan={4} className="p-12 text-center text-emerald-600"><RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2" /><span className="text-xs">Carregando...</span></td></tr>
                    ...
                    {xmlTotalPages > 0 && (
                        <div className="p-2 border-t border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
                            <span className="text-[10px] text-slate-500 pl-2">Pág. {xmlPage + 1}/{xmlTotalPages}</span>
                            <div className="flex gap-1">
                                <button onClick={() => setXmlPage(p => Math.max(0, p - 1))} disabled={xmlPage === 0 || loading} className="px-2 py-1 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 text-[10px] font-bold rounded transition-colors disabled:opacity-50">Ant</button>
                                <button onClick={() => setXmlPage(p => Math.min(xmlTotalPages - 1, p + 1))} disabled={xmlPage >= xmlTotalPages - 1 || loading} className="px-2 py-1 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 text-[10px] font-bold rounded transition-colors disabled:opacity-50">Próx</button>
                            </div>
                        </div>
                    )}
                </div>
                        ...
                        <table className="min-w-full divide-y divide-slate-100">
                            <thead className="bg-emerald-50/50 sticky top-0 z-10 shadow-sm">
                                <tr>
                                    ...
                                    <TableHeader onFilter={() => openColumnFilter('PERFIL')} isFiltered={perfilFilter.length > 0}>Perfil Fat.</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('QTD_NOTAS')} isFiltered={!!numericFilters['QTD_NOTAS']}>Qtd</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('VLR_DEVOLUCAO')} isFiltered={!!numericFilters['VLR_DEVOLUCAO']}>Devolução</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('VLR_VENDAS')} isFiltered={!!numericFilters['VLR_VENDAS']}>Vendas</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('TOTAL')} isFiltered={!!numericFilters['TOTAL']}>Líquido</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('TOTAL_ST')} isFiltered={!!numericFilters['TOTAL_ST']}>Tot. ST</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('TOTAL_TRIB')} isFiltered={!!numericFilters['TOTAL_TRIB']}>Tot. Trib</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('IMPOSTOST')} isFiltered={!!numericFilters['IMPOSTOST']}>Imp. ST</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('IMPOSTOTRIB')} isFiltered={!!numericFilters['IMPOSTOTRIB']}>Imp. Trib</TableHeader>
                                    <TableHeader align="right" onFilter={() => openColumnFilter('IMPOSTOS')} isFiltered={!!numericFilters['IMPOSTOS']}>Impostos</TableHeader>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50 bg-white">
                                {filteredParc.map((row) => {
                                    ...
                                    return (
                                        <tr key={row.CODPARC} onClick={() => setSelectedParc(isSelected ? null : row.CODPARC)} className={`cursor-pointer transition-colors duration-150 ${isSelected ? 'bg-emerald-50 text-emerald-900 border-l-4 border-l-emerald-600' : 'hover:bg-slate-50 border-l-4 border-l-transparent'}`}>
                                ...
                                {filteredParc.length === 0 && !loading && <tr><td colSpan={12} className="p-12 text-center text-slate-400 text-sm">Nenhum parceiro encontrado.<br/><button onClick={clearAllFilters} className="mt-2 px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg">Limpar Filtros</button></td></tr>}
                            </tbody>
                            {filteredParc.length > 0 && (
                                <tfoot className="bg-emerald-100/60 sticky bottom-0 z-10 shadow-[0_-1px_3px_rgba(0,0,0,0.1)] border-t-2 border-emerald-200">
                                    <tr>
                                        <td colSpan={3} className="px-4 py-3 text-right text-xs font-bold text-emerald-900 uppercase tracking-wider whitespace-nowrap">Total Geral:</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-900 tabular-nums whitespace-nowrap">{totaisParceiros.QTD_NOTAS}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-red-700 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.VLR_DEVOLUCAO)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-800 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.VLR_VENDAS)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-black text-emerald-900 tabular-nums whitespace-nowrap bg-emerald-200/40">{formatCurrency(totaisParceiros.TOTAL)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-800 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.TOTAL_ST)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-800 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.TOTAL_TRIB)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-800 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.IMPOSTOST)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-emerald-800 tabular-nums whitespace-nowrap">{formatCurrency(totaisParceiros.IMPOSTOTRIB)}</td>
                                        <td className="px-4 py-3 text-right text-sm font-black text-emerald-900 tabular-nums whitespace-nowrap bg-emerald-200/40">{formatCurrency(totaisParceiros.IMPOSTOS)}</td>
                                    </tr>
                                </tfoot>
                            )}
                        </table>
  ...
  const renderWidgetHeader = (w: WidgetConfig) => (
      <div className="px-4 py-3 border-b border-slate-200 bg-emerald-50 flex flex-wrap justify-between items-center gap-2 shrink-0">
          <h2 className="text-sm md:text-base font-bold text-slate-800 flex items-center gap-2">
              ...
              {w.title}
              {w.id === 'parceiros' && hasAnyFilterActive && (
          ...
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-1 shadow-sm no-drag">
              {!w.isFloating && (
                  <>
                      <button onClick={() => moveWidget(w.id, -1)} className="p-1 text-slate-400 hover:bg-slate-100 hover:text-emerald-600 rounded transition-colors" title="Mover para Esquerda/Cima"><MoveLeft className="w-4 h-4" /></button>
                      <button onClick={() => moveWidget(w.id, 1)} className="p-1 text-slate-400 hover:bg-slate-100 hover:text-emerald-600 rounded transition-colors" title="Mover para Direita/Baixo"><MoveRight className="w-4 h-4" /></button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col relative overflow-x-hidden">
        ...
        <div className="w-full max-w-[1920px] mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                <div className="flex items-center gap-2 bg-emerald-800/50 px-3 py-1.5 rounded-lg border border-emerald-600 shadow-inner">
                    ...
                    <input type="month" value={dtRef} onChange={(e) => setDtRef(e.target.value)} className="bg-transparent text-sm text-white font-bold focus:outline-none cursor-pointer [color-scheme:dark]" />
                </div>
                <button onClick={loadAll} className="p-2 bg-emerald-600 hover:bg-emerald-500 rounded-full transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 shadow-sm border border-emerald-500" title="Atualizar">
      ...
      <main className="flex-1 w-full max-w-[1920px] mx-auto p-4 md:p-6 space-y-6">
        ...
        {/* Grade de Widgets Dinâmicos */}
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 w-full items-start auto-rows-min">
            {widgets.filter(w => !w.isFloating).sort((a,b) => a.order - b.order).map(w => (
                <div 
                    ...
                    style={{ 
                        gridColumn: `span ${Math.min(w.colSpan, 3)} / span ${Math.min(w.colSpan, 3)}`,
                        height: w.id === 'parceiros' ? 600 : w.id === 'xml' ? 500 : 400 
                    }}
                >
      ...
      {widgets.filter(w => w.isFloating).map(w => (
          <FloatingWidget key={w.id} widget={w} updateWidget={updateWidget} bringToFront={bringToFront} headerExtra={null}>
              {renderWidgetHeader(w)}
              {renderContent(w.id)}
          </FloatingWidget>
      ))}
                      ...
                      {activeFilterCol === 'PERFIL' ? (
                          ...
                          <div className="space-y-6">
                              <div className="space-y-2">
                                  ...
                                  <input type="range" min={0} max={currentMaxForSlider} step={isCurrencyActive ? "0.01" : "1"} value={modalMin || 0} onChange={(e) => setModalMin(e.target.value)} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-600" />
                              </div>
                              <div className="space-y-2">
                                  <div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Máximo</label><span className="text-xs font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded">{formatFilterDisplayValue(modalMax || currentMaxForSlider)}</span></div>
                                  <input type="range" min={0} max={currentMaxForSlider} step={isCurrencyActive ? "0.01" : "1"} value={modalMax || currentMaxForSlider} onChange={(e) => setModalMax(e.target.value)} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-600" />
                              </div>
                          </div>
                      )}
                  </div>
