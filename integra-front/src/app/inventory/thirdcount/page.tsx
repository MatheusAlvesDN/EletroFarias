...
const Page: React.FC = () => {
  ...
  const ensureProductInfo = useCallback(
    async (codProd: number) => {
      ...
      // já tem cache => não busca de novo
      if (productInfoByCodProd[key]) return;

      // já está carregando => não duplica
      ...
      try {
        // ✅ assumindo query param ?codProd=123
        ...
        const localizacao =
          toStringSafe(rec.localizacao ?? rec.LOCALIZACAO ?? rec.loc ?? rec.LOC) || null;
  ...
  const toggleRow = (inv: InventoryItem) => {
    setExpandedId((prev) => {
      ...
      if (next) {
        // abriu => carrega info do produto
        void ensureProductInfo(inv.codProd);
      }
      return next;
    });
  };
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  ...
                  <b>Detalhes</b> para ver o histórico, enviar <b>nova contagem</b> e ver as <b>informações do produto</b>.
                </Typography>
              </Box>
