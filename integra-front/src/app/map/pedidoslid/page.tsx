...
export default function PainelExpedicaoPage() {
    ...
    return (
        <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: '#f0f4f8' }}>
            <Box component="main" sx={{ flexGrow: 1, height: '100vh', overflow: 'hidden', p: 0 }}>
                <Card square sx={{ height: '100%', display: 'flex', flexDirection: 'column', border: 'none', boxShadow: 'none' }}>
                    <CardContent sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2, height: '100%', paddingBottom: '16px !important' }}>
                        ...
                        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr auto 1fr' }, gap: 2, alignItems: 'center' }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: { xs: 'center', md: 'flex-end' }, gap: 1, flexWrap: 'wrap' }}>
                                ...
                                <Button variant="outlined" startIcon={<FullscreenIcon />} onClick={() => enterFullscreenWithRotation(0)} title="Normal" sx={{ height: 40 }}>Normal</Button>
                                <Button variant="outlined" startIcon={<RotateLeftIcon />} onClick={() => enterFullscreenWithRotation(90)} title="90°" sx={{ height: 40 }}>90°</Button>
                                <Button variant="outlined" startIcon={<RotateRightIcon />} onClick={() => enterFullscreenWithRotation(-90)} title="-90°" sx={{ height: 40 }}>-90°</Button>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                            <TextField size="small" placeholder="Filtrar..." value={q} onChange={e => setQ(e.target.value)} sx={{ flexGrow: 1 }} />
                        ...
                        {/* Tabela Principal */}
                        <TableContainer component={Paper} ref={tableWrapRef} elevation={0} sx={{ flex: 1, overflowX: 'hidden', overflowY: 'auto', bgcolor: 'background.paper', '&:fullscreen': { width: '100%', height: '100%', p: 0, overflow: 'hidden' } }}>
                            <Box sx={fullScreen ? {
                                position: 'absolute', top: '50%', left: '50%', width: `${rotW}px`, height: `${rotH}px`,
                                transform: `translate(-50%, -50%) rotate(${rotation}deg)`, transformOrigin: 'center', overflowX: 'hidden', overflowY: 'auto', bgcolor: 'background.paper'
                            } : { height: '100%', overflowX: 'hidden', overflowY: 'auto' }}>

                                <Box ref={contentRef} sx={{ transform: fullScreen ? `scale(${scale})` : 'none', transformOrigin: 'top left', width: '100%' }}>
                                    <Table stickyHeader size="small">
                                        ...
                                        <TableBody>
                                            {filtered.length === 0 ? (
                                                <TableRow><TableCell colSpan={9} align="center" sx={{ py: 4 }}><Typography variant="h6" color="text.secondary">Nenhuma pendência encontrada</Typography></TableCell></TableRow>
                                            ) : (
                                                filtered.map((row) => {
                                                    ...
                                                    return (
                                                        <React.Fragment key={row.NUNOTA}>
                                                            <TableRow sx={{
                                                                bgcolor: row.BKCOLOR,
                                                                '& td': { color: row.FGCOLOR, borderColor: 'rgba(0,0,0,0.1)' },
                                                                '&:hover': { filter: 'brightness(0.95)' }
                                                            }}>
                                                                <TableCell><Typography sx={cellTextSx}>{row.AD_TIPODEENTREGA} {row.CODTIPOPER === 322 ? ' (322)' : ''}</Typography></TableCell>
                                                                <TableCell>
                                                                ...
                                                                <TableCell><Typography sx={cellTextSx} style={{ textTransform: 'uppercase' }}>{row.PARCEIRO?.substring(0, 35)}</Typography></TableCell>
                                                                <TableCell><Typography sx={cellTextSx}>{row.VENDEDOR?.split(' ')[0]}</Typography></TableCell>
                                                                <TableCell>
                                                                ...
                                                                <TableCell>
                                                                    <Typography sx={{ fontWeight: 'bold', fontSize: '1.2em' }}>{tempoDecorrido(row.DTALTER, row.HRALTER, nowMs)}</Typography>
                                                                </TableCell>
                                                            ...
                                                            {/* LINHA EXPANDIDA COM A LISTA DE ITENS */}
                                                            <TableRow>
                                                                        ...
                                                                        <Box sx={{
                                                                            ...
                                                                            {isLoadingRow ? (
                                                                                <Box sx={{ display: 'flex', justifyContent: 'center', p: 2 }}>
                                                                                    <CircularProgress size={24} sx={{ color: row.FGCOLOR }} />
                                                                                </Box>
                                                                            ) : (
                                                                                <Table size="small" aria-label="itens">
                                                                                    <TableHead>
                                                                                        <TableRow>
                                                                                    ...
                                                                                    <TableBody>
                                                                                        {cachedItems.length === 0 ? (
                                                                                            <TableRow><TableCell colSpan={4} align="center" sx={{ color: row.FGCOLOR, border: 0 }}>Nenhum item encontrado.</TableCell></TableRow>
                                                                                        ) : (
            ...
            <Snackbar open={snackbarOpen} autoHideDuration={3000} onClose={() => setSnackbarOpen(false)}><Alert severity="success" variant="filled">{snackbarMsg}</Alert></Snackbar>
        </Box>
    );
}