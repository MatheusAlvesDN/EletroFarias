...
WITH BASE AS (
  ...
  WHERE
    ...
    AND (
       ...
       OR (CAB.CODTIPOPER = 322 AND CAB.STATUSNOTA IN ('L'))
    )
...
ORDER BY
  ...
  WHERE (((CAB.CODTIPOPER = 601 OR CAB.CODTIPOPER = 325) AND CAB.CODTIPVENDA NOT IN (131, 221, 238, 239, 193, 235, 222, 241, 192, 176, 157, 162, 163, 156, 177, 159, 236, 237, 178, 161, 158, 160) AND (CAB.AD_LIBERABOLETO = 'S' OR CAB.AD_LIBERACAIXA = 'S'))
    ...
    AND PRO.CODGRUPOPROD IN (7101906,7101104, 7101115, 7101113, 7101103, 7101102, 7101106, 7101107, 7101112, 7101105, 7101109, 7103605, 7101108, 7105405, 7101101, 7101114)
