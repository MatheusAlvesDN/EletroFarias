...
export class PrismaService {
  ...
  async getSeparadores() {
    // Optimization: Removed fetching all users to memory and filtering in JS.
    // Using Prisma's where clause directly is more efficient and avoids unnecessary memory overhead.
  ...
  async getSolicitacao() {
    // Optimization: Moved the `.filter((s) => s.aprovado === false)` to a Prisma `where` clause.
    // This avoids fetching all rows into memory and filtering in JavaScript, improving DB and node performance.
