...
export class SyncService {
  ...
  async registerClub() {
    ...
    // Optimized: Avoid fetching all consumers. Use individual checks with caching.
    const cacheExistencia = new Map<string, boolean>();
    const checkFidelimax = async (cpf: string | undefined | null) => {
      const cleanCpf = String(cpf || '').replace(/\D/g, '');
      if (!cleanCpf) return false;

      if (cacheExistencia.has(cleanCpf)) return cacheExistencia.get(cleanCpf)!;

      const exists =
        await this.fidelimaxService.verificarSeConsumidorExiste(cleanCpf);
      cacheExistencia.set(cleanCpf, exists);
      return exists;
    };
    ...
    for (const note of validClientNotesDevol) {
      ...
      const hasFidelimax = await checkFidelimax(cliente?.cpf);
    ...
    for (const note of validVendTecNotesDevol) {
      ...
      const clientHasFidelimax = await checkFidelimax(cliente?.cpf);
      ...
      console.log(vendTec);
      const vendTecHasFidelimax = await checkFidelimax(vendTec?.cpf);
      await this.sankhyaService.inFidelimaxNoteCheck(note.NUNOTA, token);
