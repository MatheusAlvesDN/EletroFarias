...
export class SyncService {
  ...
  async registerClub() {
    ...
    // Otimização: remove busca de todos consumidores (lento) e usa verificação pontual com cache
    // const fidelimaxClients = await this.fidelimaxService.listarTodosConsumidores();
    const cpfCache = new Map<string, boolean>();
    const checkCpf = async (cpf: string | undefined | null) => {
      if (!cpf) return false;
      if (cpfCache.has(cpf)) return cpfCache.get(cpf)!;
      const exists = await this.fidelimaxService.checkClienteExiste(cpf);
      cpfCache.set(cpf, exists);
      return exists;
    };
    ...
    for (const note of validClientNotesDevol) {
      ...
      const hasFidelimax = await checkCpf(cliente?.cpf);
    ...
    for (const note of validVendTecNotesDevol) {
      ...
      const clientHasFidelimax = await checkCpf(cliente?.cpf);
      ...
      console.log(vendTec);
      const vendTecHasFidelimax = await checkCpf(vendTec?.cpf);
      await this.sankhyaService.inFidelimaxNoteCheck(note.NUNOTA, token);
