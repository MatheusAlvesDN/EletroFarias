...
export class SyncService {
  ...
  async registerClub() {
    ...
    const chunkSize = 50;
    for (let i = 0; i < notasNaoPontua.length; i += chunkSize) {
      const chunk = notasNaoPontua.slice(i, i + chunkSize);
      await Promise.all(
    ...
    for (let i = 0; i < notasDevolNaoPontua.length; i += chunkSize) {
      const chunk = notasDevolNaoPontua.slice(i, i + chunkSize);
      await Promise.all(
        chunk.map(async (note) => {
          console.log(
            'nota não pontua ' +
