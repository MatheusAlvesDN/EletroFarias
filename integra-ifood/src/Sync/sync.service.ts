...
export class SyncService {
  ...
  async registerClub() {
    ...
    // Helper to normalize documents (CPF/CNPJ)
    const normalizeDoc = (doc?: string | null) =>
      doc ? doc.replace(/\D/g, '') : '';

    const fidelimaxClients =
      await this.fidelimaxService.listarTodosConsumidores();
    // Use normalized documents for matching
    const fidelimaxDocSet = new Set(
      fidelimaxClients.map((c) => normalizeDoc(c.documento)),
    );
    ...
    for (const note of validClientNotesDevol) {
      console.log('Processando devolucao sem vendTec: ' + note.NUNOTA);
      const rawCliente = await this.sankhyaService.getCPFwithCodParc(
        note.CODPARC,
        token,
      );
      const docCliente = normalizeDoc(rawCliente?.cpf);

      const hasFidelimax = docCliente ? fidelimaxDocSet.has(docCliente) : false;

      if (hasFidelimax) {
        console.log('Cliente possui cadastro no Fidelimax, tentando debitar');
        try {
          ...
          if (result.CodigoResposta == 100) {
            ...
            const userDebit = await this.prismaService.findDebit(docCliente);
            if (!userDebit) {
              await this.prismaService.registerDebit(
                docCliente,
                note.VLRNOTA,
                'Devolução TOP 800, 801',
                rawCliente?.nome || 'Desconhecido',
                String(note.NUNOTA),
              );
            } else {
            ...
            console.log(
              'Erro ao debitar (retorno inesperado): ',
              result.CodigoResposta,
            );
            // Do not mark note as processed
          }
        } catch (err: any) {
          console.error(
            'Erro na requisição de debito Fidelimax:',
            err.message || err,
          );
          // Do not mark note as processed
        }
        ...
        console.log(
          'Cliente não possui cadastro no Fidelimax. Nota marcada como lida e pontos descartados.',
        );
        await this.sankhyaService.inFidelimaxNoteCheck(note.NUNOTA, token);
      }
    }
    ...
    for (const note of validVendTecNotesDevol) {
      console.log('Processando devolucao com vendTec: ' + note.NUNOTA);
      ...
      const docCliente = normalizeDoc(rawCliente?.cpf);
      const clientHasFidelimax = docCliente
        ? fidelimaxDocSet.has(docCliente)
        : false;

      // Dados Vendedor Técnico
      const codeParcVendTec = await this.sankhyaService.getVendedor(
        note.CODVENDTEC,
        token,
      );
      const rawVendTec = await this.sankhyaService.getCPFwithCodParc(
        Number(codeParcVendTec?.CODPARC),
        token,
      );
      const docVendTec = normalizeDoc(rawVendTec?.cpf);
      const vendTecHasFidelimax = docVendTec
        ? fidelimaxDocSet.has(docVendTec)
        : false;

      let clienteProcessed = false;
      let vendTecProcessed = false;

      // Debitar Cliente
      if (clientHasFidelimax) {
        console.log(
          'Cliente possui cadastro no Fidelimax, tentando debitar (devolução)',
        );
        try {
          const result = await this.fidelimaxService.debitarConsumidor(
            docCliente,
          ...
          if (result.CodigoResposta == 100) {
            ...
            console.log('Erro ao debitar cliente: ', result.CodigoResposta);
          }
        } catch (err: any) {
          console.error(
            'Erro na requisição de debito cliente:',
            err.message || err,
          );
        }
      } else {
        console.log('Cliente não possui cadastro no Fidelimax (devolução)');
        clienteProcessed = true;
      }
      ...
      if (vendTecHasFidelimax) {
        console.log(
          'Vendedor tec. possui cadastro no Fidelimax, tentando debitar (devolução)',
        );
        try {
          const result = await this.fidelimaxService.debitarConsumidor(
            docVendTec,
          ...
          if (result.CodigoResposta == 100) {
            console.log('Estornado do vendedor.');
            vendTecProcessed = true;
          } else if (result.CodigoResposta == 113) {
            console.log('Vendedor sem saldo para estorno:', note.NUNOTA);
            const userDebit = await this.prismaService.findDebit(docVendTec);
            if (!userDebit) {
              await this.prismaService.registerDebit(
                docVendTec,
                note.VLRNOTA * 3,
                'Devolução TOP 800, 801',
                rawVendTec?.nome || 'Desconhecido',
                String(note.NUNOTA),
              );
            } else {
            ...
            console.log('Erro ao debitar vendedor: ', result.CodigoResposta);
          }
        } catch (err: any) {
          console.error(
            'Erro na requisição de debito vendedor:',
            err.message || err,
          );
        }
      } else {
        console.log(
          'Vendedor tec não possui cadastro no Fidelimax (devolução)',
        );
        vendTecProcessed = true;
      }

      // A nota só é considerada processada se as requisições de débito permitirem
      // (se ambos não tiverem cadastro ou tiverem respondido 100/113)
      if (clienteProcessed && vendTecProcessed) {
        await this.sankhyaService.inFidelimaxNoteCheck(note.NUNOTA, token);
      }
    }
    //#endregion
    ...
    for (const note of validClientNotes) {
      console.log('Processando venda sem vendTec: ' + note.NUNOTA);
      const rawCliente = await this.sankhyaService.getCPFwithCodParc(
        note.CODPARC,
        token,
      );
      ...
      // Marca lida antes de prosseguir com pontuação
      await this.sankhyaService.inFidelimaxNoteCheck(note.NUNOTA, token);
      ...
      if (hasFidelimax) {
        ...
        if (userDebit) {
          ...
          if (clientNewDebit > 0) {
            ...
            if (clientNewDebit < 0) {
              // sobrou crédito para pontuar
              await this.fidelimaxService.pontuarClienteFidelimax(
                docCliente,
                Math.abs(clientNewDebit),
                String(note.NUNOTA),
              );
            }
            // se clientNewDebit === 0, apenas quitou a dívida, não pontua
          }
        } else {
    ...
    for (const note of validVendTecNotes) {
      console.log('Processando venda com vendTec: ' + note.NUNOTA);
      // Dados Cliente
      const rawCliente = await this.sankhyaService.getCPFwithCodParc(
        note.CODPARC,
        token,
  ...
  async getNotFoundListSup(localizacao: string, codProd: number) {
    ...
    if (!notFound) {
      ...
      const faltandoSet = new Set<number>(
        (notFound.codProdFaltando ?? []) as number[],
      );
      const contadosSet = new Set<number>(
        (notFound.codProdContados ?? []) as number[],
      );
