...
export class SyncService {
  ...
  async registerClub() {
    ...
    // ⚡ Bolt Optimization: Batch network requests using `Promise.all` in `registerClub`.
    // Iterating sequentially blocks the event loop on I/O.
    // Grouping notes into chunks drastically reduces the number of discrete waiting periods.
    // Expected Impact: Massive speed up on processing times.
    ...
    const chunkedNotasNaoPontua = chunkArray(notasNaoPontua, 50);
    for (const chunk of chunkedNotasNaoPontua) {
      await Promise.all(
    ...
    const chunkedNotasDevolNaoPontua = chunkArray(notasDevolNaoPontua, 50);
    for (const chunk of chunkedNotasDevolNaoPontua) {
      await Promise.all(
        chunk.map(async (note) => {
          console.log(
            'nota não pontua ' +
