...
export class SyncService {
  ...
  async deletarNaoConfirmadas() {
    ...
    const validNunotas = notas
      .map((row) => Number(row?.[0] ?? row?.NUNOTA))
      .filter((nunota) => Number.isFinite(nunota));

    // Process in chunks of 20 to avoid overwhelming the server and N+1 sequential waiting
    const chunkSize = 20;
    for (let i = 0; i < validNunotas.length; i += chunkSize) {
      const chunk = validNunotas.slice(i, i + chunkSize);
      await Promise.all(
