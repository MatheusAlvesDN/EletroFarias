...
export class SankhyaService {
  ...
  async getPrecosProdutosTabelaBatch(
    ...
    // ⚡ Bolt: Use Promise.all with chunking to fetch product prices concurrently, improving throughput
    for (let i = 0; i < codigosProdutos.length; i += BATCH_SIZE) {
      const batch = codigosProdutos.slice(i, i + BATCH_SIZE);

      const batchPromises = batch.map(async (codProd) => {
        try {
