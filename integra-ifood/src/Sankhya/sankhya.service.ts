...
export class SankhyaService {
  ...
  async getPrecosProdutosTabelaBatch(
    ...
    // ⚡ Bolt Optimization: Batch processing using native chunking with Promise.all
    // Processes 20 requests concurrently to significantly reduce total waiting time
    // compared to sequential execution, while avoiding rate limits.
    const chunkSize = 20;

    for (let i = 0; i < codigosProdutos.length; i += chunkSize) {
      const chunk = codigosProdutos.slice(i, i + chunkSize);
      const chunkPromises = chunk.map(async (codProd) => {
        try {
